...
// Replace the Try Demo button
content = content.replace(
  /<button\s+onClick=\{\(\) => handleLaunchDemo\('rustic'\)\}\s+className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-blue-50\/80 hover:bg-blue-900\/30 text-blue-400 text-xs font-semibold transition-all cursor-pointer active:scale-95 border border-blue-900\/50\/50"\s*>\s*<Sparkles className="w-3.5 h-3.5" \/>\s*<span>Try Demo<\/span>\s*<\/button>/g,
  ''
);
...
console.log('Patched auth header 2');
